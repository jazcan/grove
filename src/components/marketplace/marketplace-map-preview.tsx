...
type Props = {
  ...
  /** "search" = positions from lat/lng vs search center; "preview" = decorative spread. */
  mode?: "search" | "preview";
...
/**
 * Map-style panel for marketplace results (no tile SDK). Pins use either geographic layout or a stable preview spread.
 */
export function MarketplaceMapPreview({ markers, mode = "preview", searchHint }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-[1] flex items-center justify-between gap-2 px-4 py-3 sm:px-5">
        <p className="text-xs font-semibold text-[var(--muted)]">{isSearch ? "Search area" : "Map view"}</p>
        <p className="text-xs text-[var(--muted)]">
          {markers.length > 0 ? `${markers.length} on map` : isSearch ? "No pins" : "Preview"}
        </p>
      </div>
      {searchHint ? (
        <p className="relative z-[1] px-4 pb-1 text-[0.65rem] leading-snug text-[var(--muted)] sm:px-5 sm:text-xs">
          {searchHint}
        </p>
      ) : null}
        ...
        {markers.length === 0 ? (
          ...
          <>
            ...
            <p className="pointer-events-none absolute bottom-3 left-0 right-0 z-[1] px-4 text-center text-[0.65rem] leading-snug text-[var(--muted)] sm:text-xs">
              {isSearch
                ? "Pin positions reflect your search area (approximate layout, not street-level tiles)."
                : "Add a location to see providers plotted by distance."}
            </p>
          </>
        )}
      </div>
